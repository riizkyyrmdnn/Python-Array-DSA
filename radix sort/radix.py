...
def int_radix_sort(array):
    ...
    while _max // exponent > 0:
        counting_sort(array, exponent)
        exponent *= 10

    return array

def float_radix_sort(array):
    if not array:
        return []
    ...
    sorted_int_array = int_radix_sort(int_array)
    return [i / factor for i in sorted_int_array]

def string_radix_sort(array):
    if not array:
        return []
...
def switch(choice):
    if choice == 1:
        ...
        print(f"After sort: {int_radix_sort(int_array)}")
    elif choice == 2:
        ...
        print(f"After sort: {float_radix_sort(float_array)}")
    elif choice == 3:
        ...
        print(f"After sort: {string_radix_sort(string_array)}")
    else:
        print("0. Close program\n")


if __name__ == "__main__":
    print("___ Radix Sort ___")
